  ...
  return (
    <div className="space-y-8 max-w-lg">
      ...
      <form action={updateProduct.bind(null, id)} className="space-y-5 bg-white rounded-2xl border border-stone-200 p-5">
        ...
        <ConditionsWithLibrary defaultValue={product.conditions_utilisation || ""} />
            ...
            {photosWithUrl.map((media) => (
              <div key={media.id}
                ...
                <div className="absolute top-1 left-1 right-1 z-10 flex items-start justify-between gap-1">
                  {!media.video_url && (
                    media.is_main
                      ? <span className="text-xs bg-lotus-600 text-white px-1.5 py-0.5 rounded-full font-medium leading-tight">Principale</span>
                      : <form action={setMainPhoto.bind(null, id, media.id)}>
                          <button type="submit" className="text-xs bg-white/80 text-stone-600 px-1.5 py-0.5 rounded-full hover:bg-lotus-600 hover:text-white transition leading-tight">
                            Principale ?
                          </button>
                        </form>
                  )}
                  <form action={deletePhoto.bind(null, id, media.id, media.storage_path || "")} className="ml-auto">
                    <button type="submit" className="w-5 h-5 rounded-full bg-black/50 text-white text-xs flex items-center justify-center hover:bg-red-600 transition">
                      X
                    </button>
                  </form>
                </div>
