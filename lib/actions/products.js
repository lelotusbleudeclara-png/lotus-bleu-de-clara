...
function parseProductFields(formData) {
  ...
  return { name, description, price, categoryId, subcategoryId, inStock, published, conditionsUtilisation, labels };
}

export async function createProduct(formData) {
  const { name, description, price, categoryId, subcategoryId, inStock, published, conditionsUtilisation, labels } = parseProductFields(formData);
...
export async function updateProduct(id, formData) {
  ...
  redirect(`/vendeur/produits/${id}?saved=1`);
}
